exports.run = (client, message, args, p) => {
    ...
    if(args[0]) {
        ...
        if(firstArgs === "yozora" || firstArgs === "y") {
            let pApi = cfg.yozoraProfileApi;
            let tApi = cfg.yozoraTopApi;
            let rApi = cfg.yozoraRecentApi;
            let pUrl = cfg.yozoraPUrl;
            let aviUrl = cfg.yozoraAviUrl;
            let nameIndex = 2;
            let server = "yozora";
            ...
            sendPrivServerHelp(message, "Yozora", p);
            return;
        } else
...
**Level**: ${bodylevel} | **Playcount**: ${bodyplaycount}`,
        ...
        if(firstArgs === 'set' || firstArgs === 's') {
            ...
            if(!args[1] || !args[2]) return message.channel.send(`Usage: ${p}osu set <bancho|akatsuki|ripple|gatari|kurikku|enshi|enjuu|yozora|kawata> <name>`);
            ...
            if(!client.osuNames.has(message.author.id, prop)) {
                return message.channel.send(`Usage: ${p}osu set <bancho|akatsuki|ripple|gatari|kurikku|enshi|enjuu|yozora|kawata> <name>`);
            }
